<?php
     ...
     **/
     ...
     class homepageAdminController extends homepage {
         ...
         * @brief 카페 설정
         **/
        function procHomepageAdminInsertConfig($vars = NULL) {
            global $lang;
        ...
        function insertHomepage($title, $domain, $isPackageDefault = true) {
            $oModuleController = &getController('module');
            ...
            if($isPackageDefault)
            {
                ...
                $info->module->notice_mid = $this->makeBoard($info->site_srl, 'notice', 'Announcement', $info->layout_srl,$info->mlayout_srl);
                $info->module->freeboard_mid = $this->makeBoard($info->site_srl, 'freeboard', '$user_lang->freeboard', $info->layout_srl,$info->mlayout_srl);
                $info->module->guestbook_mid = $this->makeModule('guestbook', 'xe_package', $info->site_srl, 'packageGuestbook', 'Guestbook', $info->layout_srl, $info->mlayout_srl);
            }
            else
            {
                $info->module->home_mid = $this->makePage($info->site_srl, $info->module->home_srl, 'home', '$user_lang->home', $info->layout_srl, $this->getHomeContent(),$info->mlayout_srl);
            ...
            if($isPackageDefault)
            {
                ...
                $this->insertMenuItem($info->menu_srl, 0, $info->module->notice_mid, 'Announcement');
                $this->insertMenuItem($info->menu_srl, 0, $info->module->freeboard_mid, '$user_lang->freeboard');
                $this->insertMenuItem($info->menu_srl, 0, $info->module->guestbook_mid, 'Guestbook');
            }
            else
            {
                $this->insertMenuItem($info->menu_srl, 0, $info->module->home_mid, '$user_lang->home');
        ...
        function makeModule($module, $skin, $site_srl, $mid, $browser_title, $layout_srl, $mlayout_srl=0) {
