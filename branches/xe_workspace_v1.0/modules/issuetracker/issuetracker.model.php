<?php
    ...
    class issuetrackerModel extends issuetracker {
        ...
        function getIssuesCount($module_srl, $target = NULL, $value = null, $status = null) {
            $args->module_srl = $module_srl;
            if($target !== NULL) $args->{$target} = $value;
		...
		function _transTarget($target, $keyword, &$args)
		{
            foreach($target as $val)
            {
                ...
                {
                    case 'title_content' :
                            $args->s_title = $keyword;
                            $args->s_content = $keyword;
                            $queryId = 'issuetracker.getIssueList';
                        break;
                    case 'title' :
                    case 'content' :
                    case 'user_id' :
                    ...
                    case 'tag' :
                            $args->s_tags = str_replace(' ','%',$keyword);
                            return 'issuetracker.getIssueListWithinTag';
                        break;
                    default :
                            preg_match('/^extra_vars([0-9]+)$/',$target,$matches);
                            if($matches[1]) {
                                $args->var_idx = $matches[1];
                                $args->var_value = str_replace(' ','%',$keyword);
                                $queryId = 'issuetracker.getIssueListWithExtraVars';
                            }
                        break;
                }
            }
        ...
        function getIssueList($args) {
            ...
            if(is_array($args->search_target) && $args->search_keyword)
            {
                $queryId = $this->_transTarget($args->search_target, $args->search_keyword, &$args);
                if(!empty($queryId))
                {
                    $query_id = $queryId;
                }
            ...
            // 검색 옵션 정리
            if(!is_array($args->search_target) && $args->search_target && $args->search_keyword) {
        ...
        function getList($module_srl, $listname, $paged = NULL, $order_type = 'desc')
        {
            if(!$module_srl) return array();

            if($paged) $pagedStr = 'Paged';

            $args->module_srl = $module_srl;
            $args->page = $paged;
            $args->order_type = $order_type;
            $output = executeQueryArray("issuetracker.get".$listname.$pagedStr, $args);
            if($paged) return $output;
		...
		function getPackageImagePath($pSrl)
		{
		    $issueImagePath = 'files/attach/package_logo/';
		    return $issueImagePath.$pSrl.'.jpg';
		}

		function getPackageImages($pSrl)
		{
		    $logo_src = $this->getPackageImagePath($pSrl);
            if(file_exists(_XE_PATH_.$logo_src)) $logo_src = $logo_src.'?rnd='.filemtime(_XE_PATH_.$logo_src);
            else $logo_src = $this->module_path.'tpl/images/packageLogo.gif';
        ...
        function getPackageList($module_srl, $package_srl=0, $each_releases_count = 0, $isPage = FALSE, $page = 1, $list_count = 100)
        {
            if($isPage)
            {
                $args->page = $page;
