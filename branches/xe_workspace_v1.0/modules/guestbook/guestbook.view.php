		...
		function dispGuestbookContentList(){
			...
			if($output->data){
				foreach($output->data as $guestbook_item){
					...
					$guestbook_list[$guestbook_item->guestbook_item_srl]->count = 0;
					...
					if($output2->data){
						foreach($output2->data as $guestbook_child_item){
							...
							$count = $guestbook_list[$guestbook_item->guestbook_item_srl]->count;
							$guestbook_list[$guestbook_item->guestbook_item_srl]->count = $count+1;
						}
					}
				}
			}
			...
			$obj = NULL;
			$obj->guestbook_item_srl = array_keys($guestbook_list);
			$targs = $oGuestbookModel->getTagList($obj);
			$voteInfo = $oGuestbookModel->getVoteByGSrls($obj->guestbook_item_srl);

            Context::set('vote_list', $voteInfo->data);
            Context::set('targs_list', $targs->data);
