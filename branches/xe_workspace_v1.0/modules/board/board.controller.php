<?php
    ...
    class boardController extends board {
        ...
        function _addExt($oldName, $fileName)
        {
            ...
            $ext = substr($oldName, (strlen ($oldName)) - (strlen (strrchr($oldName,'.'))));
            return $fileName.$ext;
        }
        ...
        function procBoardUploadFile()
        {
            ...
            if(!$upload_target_srl) return new Object(-1, "msg_invalid_request");

            //rename
            $fileName = Context::get('file_name');
            if($fileName)
            {
                $oldName = $file_info['name'];
                $fileName = $this->_addExt($oldName, $fileName);
                $file_info['name'] = $fileName;
            }

            // document module의 controller 객체 생성
            $oController = &getController('file');
            $output = $oController->insertFile($file_info, $module_srl, $upload_target_srl);
            $output = $oController->setFilesValid($upload_target_srl);

            //update the upload count number
            $oDocumentController = &getController('document');
            $oDocumentModel = &getModel('document');
            $oDocument = $oDocumentModel->getDocument($upload_target_srl);
            $uploadCount = $oDocument->get('uploaded_count');

            $obj = NULL;
            $obj->uploaded_count = $uploadCount + 1;
            $obj->document_srl = $upload_target_srl;
            $obj->title = $oDocument->get('title');
            $obj->content = $oDocument->get('content');
            $output = $oDocumentController->updateDocument($oDocument, $obj);

            $returnUrl = Context::get('success_return_url') ? Context::get('success_return_url') : getNotEncodedUrl('', 'act', 'dispBoardContent','document_srl',$upload_target_srl);
        ...
        function procBoardDeleteFile()
        {
            ...
            if(!$file_info) return new Object(-1, "msg_invalid_request");

            // document module의 controller
            $oController = &getController('file');
            $output = $oController->deleteFile($file_srl);
            ...
            $oDocumentController = &getController('document');
            $oDocumentModel = &getModel('document');
            $upload_target_srl = $file_info->upload_target_srl;
            $oDocument = $oDocumentModel->getDocument($upload_target_srl);
            $uploadCount = $oDocument->get('uploaded_count');

            $obj = NULL;
            $obj->uploaded_count = $uploadCount - 1;
            if($obj->upload_count < 0) $obj->uploaded_count = 0;
            $obj->document_srl = $upload_target_srl;
            $obj->title = $oDocument->get('title');
            $obj->content = $oDocument->get('content');
            $output = $oDocumentController->updateDocument($oDocument, $obj);
        ...
        function procBoardInsertDocument() {
            ...
            //if is link module ,then test link
            if($this->module_info->skin == 'xe_board_packageLink')
            {
                $pren = "/^(https?:\/\/)?[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"])*$/";
               if(!preg_match($pren,$obj->title)) return new Object(-1, 'Error Link,please enter the validated link');
            }
