...
class Context {
	...
	function setLangType($lang_type = 'ko') {
		...
		setcookie('lang_type', $lang_type, time()+3600*24*1000, '/');
	}
		...
		foreach($charset_list as $charset) {
			...
			foreach($obj as $key=>$val) {
				...
				if(is_array($val))
				{
				    ...
				    foreach($val as $k => $v)
				    {
				        if($v && iconv($charset,$charset,$v)!=$v) $flag = false;
				    }
				    continue;
				}
			...
			if($flag) {
				...
				foreach($obj as $key => $val)
				{
				    if(is_array($val))
				    {
				        foreach($val as $k => $v)
				        {
                            $val[$k] = iconv($charset,'UTF-8',$v);
				        }
				        $obj->{$key} = $val;
				        continue;
				    }
				    $obj->{$key} = iconv($charset,'UTF-8',$val);
				}
				return $obj;
			}
		}
			...
			if(count($_GET))
			{
				foreach($_GET as $key => $val)
				{
				    if(is_array($val))
					{
					    ...
					    foreach($val as $k => $v)
					    {
					        $vars[] = $key . '[]=' . ($v ? urlencode(Context::convertEncodingStr($v)) : '');
					    }
					    continue;
					}
