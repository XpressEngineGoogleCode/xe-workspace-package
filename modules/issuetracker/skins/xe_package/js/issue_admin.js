...
function completeReleaseInsertSetting(ret_obj) {
    var error = ret_obj['error'];
    var message = ret_obj['message'];
    var url = current_url.setQuery('act','dispIssuetrackerDownload');
    location.href = url;
}
...
jQuery(function($){
	$("a.delete_file").click(function(){
		...
		var params = {};
		params["file_srl"] = $(this).attr('data'); 
		function complete(){
			window.location.reload();
		}
		exec_xml('issuetracker', 'procIssuetrackerAdminDeleteFile', params, complete, ['message'], params);
	})
